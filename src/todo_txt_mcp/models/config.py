...
class TodoMCPConfig(BaseSettings):
    ...
    todo_file_path: Path = Field(
        ...
        description="Path to the todo.txt file"
    )
    done_file_path: Optional[Path] = Field(
        default=None,
        description="Path to the done.txt file (optional)"
    )
    
    # File handling
    encoding: str = Field(
        default="utf-8",
        description="File encoding for todo.txt files"
    )
    auto_archive: bool = Field(
        default=True,
        description="Automatically move completed todos to done.txt"
    )
    ...
    @classmethod
    def from_todo_sh_config(cls, config_path: Optional[Path] = None) -> "TodoMCPConfig":
        """
    ...
    @staticmethod
    def _find_todo_sh_config() -> Optional[Path]:
    ...
    def _parse_shell_config(config_path: Path) -> dict[str, str]:
        ...
        try:
            ...
            raise ValueError(f"Failed to parse todo.sh config file {config_path}: {e}")
