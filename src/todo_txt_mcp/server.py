...
def create_server(
    ...
    todo_file_path: Optional[str] = None,
    todo_sh_config_path: Optional[str] = None,
    config: Optional[TodoMCPConfig] = None
) -> FastMCP:
    """
    ...
    if config is None:
        if todo_sh_config_path:
            ...
            config_path = Path(todo_sh_config_path) if todo_sh_config_path != "auto" else None
...
def main() -> None:
    ...
    if len(sys.argv) > 1:
        ...
        if arg.endswith(('.cfg', '/config')) or 'todo' in Path(arg).name and not arg.endswith('.txt'):
    ...
    server = create_server(
        ...
        todo_sh_config_path=todo_sh_config_path
    )
    server.run()
