...
class FileService:
    ...
    def _check_file_size(self, file_path: Path) -> None:
        ...
        if file_path.exists() and file_path.stat().st_size > self.config.max_file_size:
            raise ValueError(f"File {file_path} exceeds maximum size of {self.config.max_file_size} bytes")
    ...
    def load_todo_list(self) -> TodoList:
        ...
        try:
            ...
            raise ValueError(f"Failed to load todo list from {todo_path}: {e}")
    ...
    def save_todo_list(self, todo_list: TodoList) -> None:
        ...
        try:
            ...
            raise ValueError(f"Failed to save todo list to {todo_path}: {e}")
    ...
    def load_done_list(self) -> Optional[TodoList]:
        ...
        try:
            ...
            raise ValueError(f"Failed to load done list from {done_path}: {e}")
